    ...
    <p>
        ...
        <select name="<?php echo $this->get_field_name('item'); ?>" id="<?php echo $this->get_field_id('item'); ?>">
            ...
            <?php foreach (spp_get_dropdown() as $category) : ?>
                <option <?php echo ('c:' . (isset($instance['item']) && $category['category_id'] == $instance['item'])) ? 'selected' : '' ?> value="c:<?php echo $category['category_id']; ?>">
                    Category: <?php echo $category['category_name']; ?>
                </option>

                <?php foreach ($category['children'] as $post): ?>
                    <option <?php echo ('p:' . (isset($instance['item']) && $post['post_id'] == $instance['item'])) ? 'selected' : '' ?> value="p:<?php echo $post['post_id']; ?>">
    ...
    <div class="spp-thumbnail">
        <div class="spp-thumbnail-wrapper">
            ...
            <p class="spp_thumbnail_dropdown_wrapper">
                <label for="<?php echo $this->get_field_name('thumbnail_size'); ?>">
                    <?php echo __('Select thumbnail size:'); ?>
                </label><br>
                <select name="<?php echo $this->get_field_name('thumbnail_size'); ?>" id="<?php echo $this->get_field_id('thumbnail_size'); ?>">
                    <?php include_once('db_queries.php'); ?>
                    <?php foreach (spp_get_thumbnail_sizes() as $name => $desc) : ?>
    ...
    <p>
        ...
        <select name="<?php echo $this->get_field_name('link_to'); ?>" id="<?php echo $this->get_field_id('link_to'); ?>">
            ...
            <?php foreach ($options as $option) : ?>
                <option value="<?php echo $option; ?>" <?php echo (isset($instance['link_to']) && $option == $instance['link_to']) ? 'selected' : '' ?>><?php echo $option; ?></option>
            <?php endforeach; ?>
        </select>
    </p>
</div>
